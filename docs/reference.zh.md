...
upstream repo 可以用 `.github/workflows/mainline-fork-pr-import.yml` 自动化这条
maintainer-side import。workflow 在 fork PR `closed` 且 `merged=true` 时通过
`pull_request_target` 触发。它 checkout 受信任的 upstream base branch，在 runner 里
初始化 `github-actions[bot]` Mainline actor，先从
`refs/pull/<number>/head` 保留 PR head object，然后运行：
...
`pr-import` 只把 GitHub PR metadata 当作 locator。它会从 fork 里发现
`refs/mainline/actors/*/log`，按 `code_commit`、`code_tree`、`git_branch` 给 sealed
intent 打分，只在存在唯一最佳匹配时调用 `actor import`。`imported` 和
`already_imported` 都是成功且幂等的结果。`no_actor_logs`、`no_match`、`ambiguous`
不会改 upstream refs，只会在 PR 下更新一条 sticky comment，提醒 maintainer 让
contributor publish，或手动跑 `mainline actor import --actor ... --remote ...`。

这个 action 需要 `contents: write` 来 push Mainline refs/notes，也需要
`pull-requests: write` / `issues: write` 来更新 PR 评论。因为它用
`pull_request_target`，workflow 不能 checkout 或执行 fork 代码；仓库自带的 workflow
只 checkout upstream base ref，fork refs 只作为 Git data fetch 给 Mainline import。
...
| `mainline pr-import --fork-url <url> --head-ref <branch> --head-sha <sha>` | merged fork PR 后自动导入 contributor intent 的辅助命令。 |
